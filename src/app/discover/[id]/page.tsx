...
export default function DiscoverUserPage() {
    ...
    const [seeYouSoonLoading, setSeeYouSoonLoading] = useState(false);
    ...
    useEffect(() => {
        ...
        // Check if current user has already marked 'See You There' for this user
        async function checkSeeYouSoon() {
            try {
                const res = await fetch('/api/user/see-you-soon-i-want');
                if (!res.ok) return;
                const data = await res.json();
                if (data.users && Array.isArray(data.users)) {
                    setSeeYouSoon(data.users.some((u: any) => u._id === id));
                }
            } catch { }
        }
        checkSeeYouSoon();
    ...
    const handleSeeYouSoon = async () => {
        if (!user || user._id === currentUserId) {
            Swal.fire({
                icon: 'info',
                title: "You can't mark yourself!",
                text: 'You cannot click See You There for yourself.',
        ...
        if (seeYouSoon) {
            Swal.fire({
                ...
                text: 'You have already marked See You There for this user.',
                confirmButtonColor: '#7bb5d3',
            });
            return;
        }
        const result = await Swal.fire({
            title: `Let ${user.name} know you'll see them at the event?`,
            text: 'They will see that you want to meet them!',
            icon: 'question',
            showCancelButton: true,
            confirmButtonColor: '#7bb5d3',
            cancelButtonColor: '#aaa',
            confirmButtonText: 'Yes, See You There!',
        });
        if (!result.isConfirmed) return;
        setSeeYouSoonLoading(true);
        try {
            const res = await fetch(`/api/connections/see-you-soon`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ userId: user._id }),
            });
            if (!res.ok) throw new Error('Failed to mark See You There');
            trackSeeYouSoon(currentUserId || 'anonymous');
            setSeeYouSoon(true);
            Swal.fire({
                icon: 'success',
                title: 'See You There!',
                text: `You've let ${user.name} know you'll see them at the event!`,
                confirmButtonColor: '#7bb5d3',
            });
        } catch (err) {
            Swal.fire({
                ...
                text: 'Could not mark See You There. Please try again.',
                confirmButtonColor: '#7bb5d3',
            });
        } finally {
    ...
    if (!session) {
        return (
            <div className="text-center py-12 lumo-card bg-white rounded-lg shadow-sm">
                <h3 className="text-lg font-medium text-gray-700 mb-1">Sign in required</h3>
                <p className="mb-4 text-gray-500">You must be logged in to view user profiles.</p>
                <button
        ...
        <>
            <div className="max-w-lg mx-auto mt-8 lumo-card p-6">
                ...
                {user._id !== currentUserId ? (
                    <button
                        className={`lumo-btn lumo-btn-primary w-full py-3 px-4 rounded-lg font-medium flex items-center justify-center transition-colors ${seeYouSoon ? 'opacity-60 cursor-not-allowed' : ''}`}
                        onClick={e => { trackClick(e); handleSeeYouSoon(); }}
                        disabled={seeYouSoon || seeYouSoonLoading}
                        aria-label="See You There"
                    >
                        {seeYouSoonLoading ? (
                            <span className="flex items-center"><span className="loader mr-2"></span>Marking...</span>
                        ) : seeYouSoon ? (
                            <>
            ...
            {/* Add a simple loader style if not present */}
            <style jsx global>{`
          .loader {
            border: 2px solid #e0e0e0;
            border-top: 2px solid #7bb5d3;
            border-radius: 50%;
            width: 1em;
            height: 1em;
